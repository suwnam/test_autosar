...
case \"\$backup_type\" in
    \"initialFullBackup\")
        BACKUP_OUTPUT=\$(restic -r \$RESTIC_REPO_NEXUS backup \"\$NEXUS_HOME\" --tag \"\$BACKUP_TAG\" 2>&1)
        ;;
    \"fullBackup\")
        BACKUP_OUTPUT=\$(restic -r \$RESTIC_REPO_NEXUS backup --force \"\$NEXUS_HOME\" --tag \"\$BACKUP_TAG\" 2>&1)
        ;;
    \"incBackup\")
        BACKUP_OUTPUT=\$(restic -r \$RESTIC_REPO_NEXUS backup \"\$NEXUS_HOME\" --tag \"\$BACKUP_TAG\" 2>&1)
        ;;
